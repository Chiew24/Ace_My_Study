(() => {
  ...
  let mySubjects = JSON.parse(localStorage.getItem(SUBJECTS_KEY) || localStorage.getItem(LEGACY_KEY) || '[]').filter(subject => subject === 'Additional Mathematics' || subject === 'Mathematics');
  ...
  function showSubjectToast(message) {
    const old = document.querySelector('.subject-toast'); if (old) old.remove();
    const toast = document.createElement('div'); toast.className = 'subject-toast'; toast.textContent = message; document.body.appendChild(toast);
  ...
  function bindSubjectButtons() {
    document.querySelectorAll('.add-subject').forEach(button => {
      const added = mySubjects.includes(button.dataset.subject); button.textContent = added ? 'Added' : '+ Add'; button.classList.toggle('added', added);
      button.onmouseenter = () => { if (button.classList.contains('added')) button.textContent = 'Remove'; };
      button.onmouseleave = () => { if (button.classList.contains('added')) button.textContent = 'Added'; };
      button.onclick = event => {
        event.stopPropagation(); const subject = button.dataset.subject; const wasAdded = mySubjects.includes(subject);
        mySubjects = wasAdded ? mySubjects.filter(item => item !== subject) : [...mySubjects, subject];
        localStorage.setItem(SUBJECTS_KEY, JSON.stringify(mySubjects));
        renderMySubjects(); showSubjectToast(wasAdded ? `${subject} is removed` : `${subject} is added`);
      };
    });
  }
  ...
  function showPanel(panel) {
    const showMySubjects = panel === 'my'; mySubjectsPanel.hidden = !showMySubjects; explorePanel.hidden = showMySubjects; mySubjectsTab.classList.toggle('active', showMySubjects); exploreTab.classList.toggle('active', !showMySubjects);
  }

  mySubjectsTab.addEventListener('click', () => showPanel('my')); exploreTab.addEventListener('click', () => showPanel('explore')); renderMySubjects();
})();